"""
HMM Agent Worker — Redis Queue Consumer with Auto-Scaling

Features:
  - Supervisor process monitors queue depth and auto-scales workers
  - Workers pull jobs from tc:queue:hmm via BRPOP
  - Runs vessal_trackapi_hmm/hmm/tracker_worker.py as subprocess (Selenium)
  - POSTs Departed and ETA events to Portal API
  - Auto-retry with exponential backoff on failures

Run:  python hmm_agent.py
"""
...
# Redis Keys
QUEUE_KEY = "tc:queue:hmm"
PROCESSING_KEY = "tc:processing:hmm"
FAILED_KEY = "tc:failed:hmm"
COMPLETED_KEY = "tc:completed:hmm"
RETRY_KEY = "tc:retries:hmm"
WORKERS_KEY = "tc:workers:hmm"
...
def process_job(job_data):
    """
    Process a single HMM tracking job:
    1. Run tracker_worker.py subprocess (Selenium)
    2. Extract departure_value → Departed event (no date)
    3. Extract ETA_date/ETA_value → ETA event

    Response format: {"status": "success", "data": {"departure_value": ..., "ETA_value": ..., "ETA_date": ...}}
    """
    container_no = job_data.get("container_no", "UNKNOWN")
    api_status = job_data.get("status", "")

    # --- STEP 1: Run HMM tracker subprocess ---
    print(f"    [HMM] Tracking {container_no}...", flush=True)
    try:
        ...
        return {"success": False, "error": "HMM tracker timed out"}
    ...
    # HMM response is nested: {"status": "success", "data": {...}}
            ...
            try:
                ...
                try:
                    ...
                    if result.get("success"):
                        ...
                        print(f"  [{worker_id}] [OK] {container_no} done (events={result.get('events_posted', 0)})", flush=True)
                    else:
                        r.hdel(PROCESSING_KEY, container_no)
                        ...
                        print(f"  [{worker_id}] [RETRY] {container_no} failed (attempt {retry_count}): {result.get('error')}", flush=True)
                        time.sleep(delay)
                    ...
                    delay = RETRY_DELAYS[min(retry_count - 1, len(RETRY_DELAYS) - 1)]
                    r.hset(RETRY_KEY, container_no, retry_count)
                    print(f"  [{worker_id}] [RETRY] {container_no} crashed (attempt {retry_count}): {e}", flush=True)
                    time.sleep(delay)
                    r.lpush(QUEUE_KEY, json.dumps(job_data))
...
def supervisor():
    ...
    print(f"\n{'='*60}")
    print(f"HMM AGENT SUPERVISOR STARTED (Auto-Scaling)")
    print(f"{'='*60}")
