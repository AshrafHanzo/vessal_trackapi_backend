...
PROJECT_DIR="/root/track_container"
...
SERVICES=(
    ...
    "vessel_trackapi_kmtc/kmtc"
    "vessal_trackapi_one_line/vessal_trackapi_one_line"
    "interasia"
    "ESL"
    "vessal_trackapi_hmm/hmm"
    "wan_hai"
)
...
for svc in "${SERVICES[@]}"; do
    ...
    if [ -f "$VENV_PIP" ]; then
        ...
        $VENV_PIP install redis>=5.0.0 requests rapidfuzz 2>&1 | tail -1
        echo -e "  ${GREEN}✓ $svc${NC}"
    else
...
AGENTS=(
    ...
    ["kmtc-agent"]="vessel_trackapi_kmtc/kmtc|kmtc_agent.py"
    ["one-line-agent"]="vessal_trackapi_one_line/vessal_trackapi_one_line|one_line_agent.py"
    ["interasia-agent"]="interasia|interasia_agent.py"
    ["esl-agent"]="ESL|esl_agent.py"
    ["hmm-agent"]="vessal_trackapi_hmm/hmm|hmm_agent.py"
    ["wan-hai-agent"]="wan_hai|wan_hai_agent.py"
)
