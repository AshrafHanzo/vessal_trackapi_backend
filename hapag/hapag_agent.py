"""
Hapag Agent Worker — Redis Queue Consumer
"""
...
# Structure: hapag/hapag_agent.py
...
QUEUE_KEY = "tc:queue:hapag"
PROCESSING_KEY = "tc:processing:hapag"
COMPLETED_KEY = "tc:completed:hapag"
RETRY_KEY = "tc:retries:hapag"
WORKERS_KEY = "tc:workers:hapag"
...
def process_job(job_data):
    ...
    try:
        ...
        return {"success": False, "error": "Hapag tracker timed out"}
...
def worker_process(worker_num):
    worker_id = f"hapag-worker-{worker_num}-pid{os.getpid()}"
    r = get_redis()
...
if __name__ == "__main__":
    print(f"HAPAG AGENT STARTING (Redis: {REDIS_HOST}:{REDIS_PORT})")
    worker_process(1)
