"""
ONE LINE Agent Worker — Redis Queue Consumer with Auto-Scaling

Features:
  - Supervisor process monitors queue depth and auto-scales workers
  - Workers pull jobs from tc:queue:one_line via BRPOP
  - Runs vessal_trackapi_one_line/vessal_trackapi_one_line/one_line_tracker.py as subprocess (Playwright)
  - POSTs Departed and ETA events to Portal API
  - Auto-retry with exponential backoff on failures

Run:  python one_line_agent.py
"""
...
TRACKER_SCRIPT = os.path.join(SCRIPT_DIR, "one_line_tracker.py")
...
# Redis Keys
QUEUE_KEY = "tc:queue:one_line"
PROCESSING_KEY = "tc:processing:one_line"
FAILED_KEY = "tc:failed:one_line"
COMPLETED_KEY = "tc:completed:one_line"
RETRY_KEY = "tc:retries:one_line"
WORKERS_KEY = "tc:workers:one_line"
...
def process_job(job_data):
    """
    Process a single ONE LINE tracking job:
    1. Run one_line_tracker.py subprocess (Playwright) to scrape ONE LINE
    2. Extract departed_value → Departed event
    3. Extract eta_date/eta_value → ETA event
    """
    container_no = job_data.get("container_no", "UNKNOWN")
    api_status = job_data.get("status", "")

    # --- STEP 1: Run ONE LINE tracker subprocess ---
    print(f"    [ONE LINE] Tracking {container_no}...")
    try:
        ...
        return {"success": False, "error": "ONE LINE tracker timed out"}
    ...
    if raw_data.get("error"):
        shared_utils.post_event(container_no, "", "", "", is_status_changed=False)
        return {"success": True, "events_posted": 0, "error": raw_data["error"]}
...
def supervisor():
    ...
    print(f"\n{'='*60}")
    print(f"ONE LINE AGENT SUPERVISOR STARTED (Auto-Scaling)")
    print(f"{'='*60}")
