"""
CFS CITPL Agent Worker — Redis Queue Consumer with Auto-Scaling

Features:
  - Supervisor process monitors queue depth and auto-scales workers
  - Workers pull jobs from tc:queue:cfs via BRPOP
  - Runs vessal_trackapi_cfs/citpl_tracker.py as subprocess (Playwright)
  - POSTs CFS In and CFS Out events to Portal API
  - Auto-retry with exponential backoff on failures

Run:  python cfs_agent.py
"""
...
TRACKER_SCRIPT = os.path.join(SCRIPT_DIR, "citpl_tracker.py")
...
# Redis Keys
QUEUE_KEY = "tc:queue:cfs"
PROCESSING_KEY = "tc:processing:cfs"
FAILED_KEY = "tc:failed:cfs"
COMPLETED_KEY = "tc:completed:cfs"
RETRY_KEY = "tc:retries:cfs"
WORKERS_KEY = "tc:workers:cfs"
...
def process_job(job_data):
    """
    Process a CFS CITPL tracking job:
    1. Run citpl_tracker.py subprocess
    2. Extract CFS In and CFS Out dates
    ...
    try:
        ...
        return {"success": False, "error": "CITPL tracker timed out"}
    ...
    cfs_in = result_data.get("cfs_in")
    cfs_out = result_data.get("cfs_out")
    ...
    cfs_display = "CITPL"  # Website source name for Port In/Port Out value
    ...
    if cfs_in and get_rank(api_status) < get_rank("Port In"):
        ...
        is_changed = str(cfs_in) != str(existing_pi)
        if post_event(container_no, "Port In", cfs_in, cfs_display, cfs_code=matched_code, cfs_name=matched_name, is_status_changed=is_changed):
            if is_changed: events_posted += 1
            print(f"    [MATCH] Port In = {cfs_in} (Changed: {is_changed})")

    if cfs_out and get_rank(api_status) < get_rank("Port Out"):
        existing_po = status_details.get("Port Out", {}).get("date")
        is_changed = str(cfs_out) != str(existing_po)
        if post_event(container_no, "Port Out", cfs_out, cfs_display, cfs_code=matched_code, cfs_name=matched_name, is_status_changed=is_changed):
            if is_changed: events_posted += 1
            print(f"    [MATCH] Port Out = {cfs_out} (Changed: {is_changed})")

    # Post CFS In / CFS Out events (Customs section in UI)
    if matched_code and get_rank(api_status) < get_rank("CFS In"):
        dt = cfs_in or cfs_out or ""
...
def supervisor():
    r = get_redis()
    print(f"\n{'='*60}")
    print(f"CFS CITPL AGENT SUPERVISOR STARTED (Auto-Scaling)")
    print(f"{'='*60}")
