"""
ESL (Emirates Shipping Line) Agent Worker — Redis Queue Consumer with Auto-Scaling

Features:
  - Supervisor process monitors queue depth and auto-scales workers
  - Workers pull jobs from tc:queue:esl via BRPOP
  - Runs ESL/tracker_worker.py as subprocess (Playwright + EasyOCR captcha)
  - POSTs Departed and ETA events to Portal API
  - Auto-retry with exponential backoff on failures
  - Extended timeout due to captcha solving (up to 30 retries)

Run:  python esl_agent.py
"""
...
# Redis Keys
QUEUE_KEY = "tc:queue:esl"
PROCESSING_KEY = "tc:processing:esl"
FAILED_KEY = "tc:failed:esl"
COMPLETED_KEY = "tc:completed:esl"
RETRY_KEY = "tc:retries:esl"
WORKERS_KEY = "tc:workers:esl"

# Timeouts — ESL has captcha (up to 30 retries), so we need a longer timeout
BRPOP_TIMEOUT = 30
SCRAPER_TIMEOUT = 300      # 5 minutes max (captcha can take a while)
...
def process_job(job_data):
    """
    Process a single ESL tracking job:
    1. Run tracker_worker.py subprocess (Playwright + EasyOCR captcha)
    ...
    # --- STEP 1: Run ESL tracker subprocess ---
    print(f"    [ESL] Tracking {container_no}...", flush=True)
    try:
        ...
        return {"success": False, "error": "ESL tracker timed out (captcha?)"}
    ...
    if data.get("status") == "error" or data.get("status") == "not_found":
        shared_utils.post_event(container_no, "", "", "", is_status_changed=False)
        return {"success": True, "events_posted": 0, "error": data.get("message", "Tracker returned error")}
...
def supervisor():
    ...
    print(f"\n{'='*60}")
    print(f"ESL AGENT SUPERVISOR STARTED (Auto-Scaling)")
    print(f"{'='*60}")
    ...
    print(f"  Scraper timeout: {SCRAPER_TIMEOUT}s (captcha)")
