...
def process_job(job_data):
    ...
    if mode == "port":
        ...
        if port_in:
            ...
            is_changed = str(port_in) != str(existing_pi)
            if post_event(container_no, "Port In", port_in, port_name, is_status_changed=is_changed):
                if is_changed: events_posted += 1

        if port_out:
            existing_po = status_details.get("Port Out", {}).get("date")
            is_changed = str(port_out) != str(existing_po)
            if post_event(container_no, "Port Out", port_out, port_name, is_status_changed=is_changed):
                if is_changed: events_posted += 1
        ...
        if cfs_in:
            ...
            is_changed = str(cfs_in) != str(existing_ci)
            if post_event(container_no, "CFS In", cfs_in, cfs_name, is_status_changed=is_changed):
                if is_changed: events_posted += 1

        if cfs_out:
            existing_co = status_details.get("CFS Out", {}).get("date")
            is_changed = str(cfs_out) != str(existing_co)
            if post_event(container_no, "CFS Out", cfs_out, cfs_name, is_status_changed=is_changed):
                if is_changed: events_posted += 1
