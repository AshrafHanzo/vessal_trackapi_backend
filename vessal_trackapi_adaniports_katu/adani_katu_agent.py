"""
Adani Kattupalli Agent Worker — Redis Queue Consumer with Auto-Scaling

Features:
  - Supervisor process monitors queue depth and auto-scales workers
  - Workers pull jobs from tc:queue:adani_katu via BRPOP
  - Runs vessal_trackapi_adaniports_katu/adani_tracker.py as subprocess (Playwright)
  - POSTs CFS In (entry_time) and CFS Out (exit_time) events to Portal API
  - Auto-retry with exponential backoff on failures

Run:  python adani_katu_agent.py
"""
...
# Redis Keys
QUEUE_KEY = "tc:queue:adani_katu"
PROCESSING_KEY = "tc:processing:adani_katu"
FAILED_KEY = "tc:failed:adani_katu"
COMPLETED_KEY = "tc:completed:adani_katu"
RETRY_KEY = "tc:retries:adani_katu"
WORKERS_KEY = "tc:workers:adani_katu"
...
def process_job(job_data):
    """
    Process an Adani Kattupalli tracking job:
    ...
    try:
        ...
        return {"success": False, "error": "Adani Katu tracker timed out"}
    ...
    cfs_display = "Adani Kattupalli"  # Website source name for Port In/Port Out value
...
def supervisor():
    r = get_redis()
    print(f"\n{'='*60}")
    print(f"ADANI KATTUPALLI AGENT SUPERVISOR STARTED (Auto-Scaling)")
    print(f"{'='*60}")
