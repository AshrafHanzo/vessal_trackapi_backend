...
app = FastAPI(title="Adani Ports Container Tracker")
...
def root():
    return {"message": "Adani Ports Tracker API is running."}

@app.get("/adani", summary="Adani Ports (Kattupalli) Search")
def adani_search(
    container_no: str = Query(..., description="Container number to search")
):
    """
    Search for a container on Adani Ports (Kattupalli).
...
if __name__ == "__main__":
    import uvicorn
    # Host on 0.0.0.0 to be accessible, port 8011 as before
