...
def run_icegate_logic(container_obj):
    ...
    # Clean port: "Chennai, IN" -> "Chennai" (take left side of comma)
    if "," in port:
        city = port.split(",")[0].strip()
    else:
        city = port.strip()
    
    # Define Explicit Icegate Port Names for the Cluster
    # If city is any of these, check ALL THREE specific ports
    cluster_triggers = ["chennai", "ennore", "kattupalli", "kamarajar"]
    
    CHENNAI_CLUSTER_PORTS = [
        "CHENNAI SEA (INMAA1)", 
        "KAMARAJAR (INENR1)", 
        "KATTUPALLI (INKAT1)"
    ]
    ...
    if city.lower() in cluster_triggers:
        print(f"  [Icegate] Detected Cluster ({city}) -> Checking {CHENNAI_CLUSTER_PORTS}")
        target_ports = CHENNAI_CLUSTER_PORTS
    else:
        print(f"  [Icegate] Checking Single Port: {city}")
        target_ports = [city]
    ...
    for check_port in target_ports:
        if found_igm or found_inward: break 
                ...
                if data:
                    ...
                    if igm_date:
                        ...
                        if get_rank(api_status) < get_rank("IGM"):
                            ...
                            # Format: "IGM [Port]" to know where it was found? Or just simple IGM?
                            # User didn't specify, sticking to standard "IGM" status
                    ...
                    # If we found data, should we stop probing other ports in the cluster?
                    # Likely yes, a container can't be at two ports simultaneously for the same IGM (usually).
                    if found_igm or found_inward:
                        break 
