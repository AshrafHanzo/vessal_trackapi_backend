...
def run():
    print("=" * 60)
    print("ORCHESTRATOR: DEPARTED & ETA (LINUX)")
    ...
    stats = {"pushed_linux": 0, "pushed_windows": 0, "skipped_other_os": 0, "skipped_status": 0, "skipped_up_to_date": 0, "errors": 0}
    
    # OS-Aware Bifurcation
    ...
    for container in containers:
        try:
            ...
            # RULE: Eligibility check — Continue until Inward status is reached
            ...
            if not service:
                print(f"  [SKIP] {container_no}: No active tracker for '{shipping_line}'")
                continue
            ...
            # RULE: Track frequently (10 min interval)
            # RULE: Validation — Skip invalid container numbers (like 23467)
            if not shared_utils.is_valid_container(container_no):
                # print(f"  [SKIP] {container_no}: Invalid container format.")
                continue
            ...
            print(f"  [PROCESS-{'WINDOWS' if IS_WINDOWS else 'LINUX'}] {container_no} (Status: {status}, Line: {shipping_line})")
            ...
            # Push to local/central queue directly for Linux-managed lines
            if shared_utils.push_to_queue(r, service, job_payload):
                print(f"    -> Pushed to {service} (Central Redis)")
                stats["pushed_linux"] += 1
            else:
    ...
    print("ORCHESTRATOR: DEPARTED & ETA (LINUX) FINISHED")
    print("=" * 60)
