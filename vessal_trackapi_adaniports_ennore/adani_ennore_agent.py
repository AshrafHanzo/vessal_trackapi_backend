...
def process_job(job_data):
    ...
    if entry_time and get_rank(api_status) < get_rank("Port In"):
        if post_event(container_no, "Port In", entry_time, cfs_display, cfs_code=matched_code, cfs_name=matched_name):
            events_posted += 1
            print(f"    [MATCH] Port In = {entry_time}")

    if exit_time and get_rank(api_status) < get_rank("Port Out"):
        if post_event(container_no, "Port Out", exit_time, cfs_display, cfs_code=matched_code, cfs_name=matched_name):
            events_posted += 1
            print(f"    [MATCH] Port Out = {exit_time}")
    ...
    if matched_code and get_rank(api_status) < get_rank("CFS In"):
        if post_event(container_no, "CFS In", entry_time or exit_time or "", matched_code):
            events_posted += 1
            print(f"    [MATCH] CFS In (CFS Code) = {matched_code}")

    if matched_name and get_rank(api_status) < get_rank("CFS Out"):
        if post_event(container_no, "CFS Out", entry_time or exit_time or "", matched_name):
            events_posted += 1
