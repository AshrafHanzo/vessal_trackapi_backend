"""
Adani Ennore Agent Worker — Redis Queue Consumer with Auto-Scaling

Features:
  - Supervisor process monitors queue depth and auto-scales workers
  - Workers pull jobs from tc:queue:adani_ennore via BRPOP
  - Runs vessal_trackapi_adaniports_ennore/adani_tracker.py as subprocess (Playwright)
  - POSTs CFS In (entry_time) and CFS Out (exit_time) events to Portal API
  - Auto-retry with exponential backoff on failures

Run:  python adani_ennore_agent.py
"""
...
TRACKER_SCRIPT = os.path.join(SCRIPT_DIR, "adani_tracker.py")
...
# Redis Keys
QUEUE_KEY = "tc:queue:adani_ennore"
PROCESSING_KEY = "tc:processing:adani_ennore"
FAILED_KEY = "tc:failed:adani_ennore"
COMPLETED_KEY = "tc:completed:adani_ennore"
RETRY_KEY = "tc:retries:adani_ennore"
WORKERS_KEY = "tc:workers:adani_ennore"
...
def process_job(job_data):
    """
    Process an Adani Ennore tracking job:
    1. Run adani_tracker.py subprocess
    2. Extract entry_time (CFS In) and exit_time (CFS Out)
    ...
    try:
        ...
        return {"success": False, "error": "Adani Ennore tracker timed out"}
    ...
    cfs_display = "Adani Ennore"  # Website source name for Port In/Port Out value
    ...
    if entry_time and get_rank(api_status) < get_rank("Port In"):
        existing_pi = status_details.get("Port In", {}).get("date")
        is_changed = str(entry_time) != str(existing_pi)
        if post_event(container_no, "Port In", entry_time, cfs_display, cfs_code=matched_code, cfs_name=matched_name, is_status_changed=is_changed):
            if is_changed: events_posted += 1
            print(f"    [MATCH] Port In = {entry_time} (Changed: {is_changed})")

    if exit_time and get_rank(api_status) < get_rank("Port Out"):
        existing_po = status_details.get("Port Out", {}).get("date")
        is_changed = str(exit_time) != str(existing_po)
        if post_event(container_no, "Port Out", exit_time, cfs_display, cfs_code=matched_code, cfs_name=matched_name, is_status_changed=is_changed):
            if is_changed: events_posted += 1
            print(f"    [MATCH] Port Out = {exit_time} (Changed: {is_changed})")

    # Post CFS In / CFS Out events (Customs section in UI)
    if matched_code and get_rank(api_status) < get_rank("CFS In"):
        dt = entry_time or exit_time or ""
        existing_ci = status_details.get("CFS In", {}).get("date")
        existing_val = status_details.get("CFS In", {}).get("value")
        is_changed = str(dt) != str(existing_ci) or str(matched_code) != str(existing_val)
        if post_event(container_no, "CFS In", dt, matched_code, is_status_changed=is_changed):
            if is_changed: events_posted += 1
            print(f"    [MATCH] CFS In (CFS Code) = {matched_code} (Changed: {is_changed})")

    if matched_name and get_rank(api_status) < get_rank("CFS Out"):
        dt = entry_time or exit_time or ""
        existing_co = status_details.get("CFS Out", {}).get("date")
        existing_val = status_details.get("CFS Out", {}).get("value")
        is_changed = str(dt) != str(existing_co) or str(matched_name) != str(existing_val)
        if post_event(container_no, "CFS Out", dt, matched_name, is_status_changed=is_changed):
            if is_changed: events_posted += 1
            print(f"    [MATCH] CFS Out (CFS Name) = {matched_name} (Changed: {is_changed})")
...
def supervisor():
    r = get_redis()
    print(f"\n{'='*60}")
    print(f"ADANI ENNORE AGENT SUPERVISOR STARTED (Auto-Scaling)")
    print(f"{'='*60}")
